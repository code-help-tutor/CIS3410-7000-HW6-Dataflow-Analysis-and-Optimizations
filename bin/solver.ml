...
module type FACT =
  sig
    ...
    (* The compare function is expected to behave like compare in the
       OCaml stdlib, that is,
       1. returns 0 if the two inputs are equal
       2. returns a negative number if the first input is less than the second
       3. returns a positive number if the first input is greater than the second
    *)
    val compare : t -> t -> int
    val to_string : t -> string
  end
